...
class BlueBrain:
    def __init__(self, alpha=0.2, gamma=0.99, epsilon=0.40, min_epsilon=0.05, decay=0.005):
        ...
        # --- RASTREADORES DO POTENCIAL DINÂMICO (DPRS) ---
        self.episode_reward_max = -9999.0
        self.episode_reward_min = 9999.0
        self.active_battles_reward = {}

        # --- O HALL DA FAMA (MEMÓRIA DE ELITE) ---
        self.elite_memory = deque(maxlen=2000) # Guarda os melhores turnos da história
        self.active_battles_transitions = {} # Rastreador temporário da batalha atual
    ...
    def _update_global_records(self, final_reward):
        """Consolida os limites históricos de pontuação ao fim de cada batalha."""
    ...
    def _get_adapted_potential(self, battle, state):
        """Wrapper que decide entre o Potencial Tático Puro ou a Fusão com o Dinâmico."""
        # 1. Base Estática (Obrigatória para manter a orientação de tipos e HP no meio do jogo)
        phi_tactical = self._calculate_potential(battle, state)

        # 2. Se for MaxDamage OU o Epsilon ainda não chegou no mínimo, mantém 100% Tático
        if self.current_phase == "maxdamage" or self.epsilon > (self.min_epsilon + 0.001):
            return phi_tactical

        # 3. MODO FUSÃO (Epsilon no Mínimo contra Instinto/Selfplay)
        tag = battle.battle_tag
        current_reward = self.active_battles_reward.get(tag, 0.0)
        denom = self.episode_reward_max - self.episode_reward_min

        # Proteção para o início da coleta
        if denom < 1e-6:
            return phi_tactical

        # Cálculo da nota de performance histórica do agente
        normalized = (current_reward - self.episode_reward_max) / denom
        phi_dynamic = 1.0 + normalized
        phi_dynamic = max(0.0, min(2.0, phi_dynamic))

        # Escala para parear com a grandeza numérica do tático
        phi_dynamic_scaled = phi_dynamic * 50.0

        # FUSÃO BALANCEADA: Garante 50% de navegação tática base e 50% de incentivo de quebra de recorde
        return (0.5 * phi_tactical) + (0.5 * phi_dynamic_scaled)
    ...
    def calculate_reward(self, battle, history, current_state=None):
        ...
        if battle.won: ext_reward += 5000.0
        elif battle.lost: ext_reward -= 5000.0
        ...
        if current_my_fainted > my_fainted_prev: ext_reward -= 100.0
        if current_opp_fainted > opp_fainted_prev: ext_reward += 100.0

        tag = battle.battle_tag
        if tag not in self.active_battles_reward:
            self.active_battles_reward[tag] = 0.0
            self.active_battles_transitions[tag] = [] # Inicia a fita de gravação
        ...
        shaping_reward = phi_current - phi_prev
        history['last_phi'] = phi_current
        
        final_turn_reward = ext_reward + shaping_reward

        # --- GRAVAÇÃO DA CENA (INVISÍVEL AO AGENTE) ---
        ...
        if last_state and last_action_tuple:
            ...
            # Se for o turno de morte, o estado atual não existe, então cravamos o estado terminal
        ...
        # --- CONCLUSÃO E JULGAMENTO PARA O HALL DA FAMA ---
        if battle.won or battle.lost:
            final_ep_reward = self.active_battles_reward.pop(tag, 0.0)
            
            # A MAGIA: Se bateu o próprio recorde de pontuação de forma positiva!
            if final_ep_reward > self.episode_reward_max and final_ep_reward > 100:
                turnos_salvos = self.active_battles_transitions.get(tag, [])
                for t in turnos_salvos:
                    self.elite_memory.append(t)
                
                print(f"\n[HALL DA FAMA] 🏆 Estratégia Genial Descoberta! Recorde: {final_ep_reward:.0f} | Movimentos Retidos: {len(turnos_salvos)}")
            
            self.active_battles_transitions.pop(tag, None)
            self._update_global_records(final_ep_reward)

        # Coletor de lixo para Ghost Battles
        if len(self.active_battles_reward) > 20:
            keys_to_delete = list(self.active_battles_reward.keys())[:-10]
    ...
    def apply_epsilon_shock_if_stagnant(self, current_win_rate):
        ...
        # 2. Chegamos no piso mínimo. Agora sim começamos a gravar a janela de avaliação.    0;\M V 1q2w35e467890-  
    ...
    def replay_experience(self):
        ...
        # --- O INJETOR DO HALL DA FAMA (A SUPER-LAPIDAÇÃO) ---
        elite_sample = []
        target_elite = int(self.batch_size * 0.25) # 25% dos sonhos focados apenas na Elite
        
        if len(self.elite_memory) > 0:
            take_elite = min(target_elite, len(self.elite_memory))
            elite_sample = random.sample(self.elite_memory, take_elite)
            
        # O resto do espaço (75%) vai para as memórias normais (exploração/manutenção)
        ...
        # Alocação ajustada para o espaço normal restante
        ...
        # --- FUNDE A MEMÓRIA NORMAL COM A ELITE ---
    ...
    def save_model(self, filename="blue_brain.pkl"):
        ...
        data = {
            ...
            "alpha": getattr(self, 'alpha', self.initial_alpha),
            "current_phase": getattr(self, 'current_phase', 'maxdamage'),
            "episode_reward_max": getattr(self, 'episode_reward_max', -9999.0),
            "episode_reward_min": getattr(self, 'episode_reward_min', 9999.0),
            # --- SALVANDO A ELITE ---
            "elite_memory": getattr(self, 'elite_memory', deque(maxlen=2000))
        }
        try:
            with open(temp_filepath, "wb") as f: pickle.dump(data, f)
            ...
            try:
                with open(filepath, "rb") as f:
                    ...
                    if saved_phase == self.current_phase:
                        ...
                        print(f"[CÉREBRO] Nova fase detectada ({saved_phase} -> {self.current_phase})!")
                    ...
                    print(f"[CÉREBRO] Brain Carregado. Estados: {len(self.q_table)} | Fase: {self.current_phase.upper()} | Elite Retida: {len(self.elite_memory)}")
