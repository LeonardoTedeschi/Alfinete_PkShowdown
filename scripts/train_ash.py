"""
scripts/train_ash.py — treino individual do agente ASH (Q-Learning com o minimo indispensavel).
...
# WARM RESTART do alfa (Loshchilov & Hutter 2016, SGDR).
# No inicio de cada sessao o alfa e reposto neste valor, criando um pico periodico de
# taxa de aprendizado que ajuda a sair de otimos locais. O primeiro decay_epsilon do
# bloco recalcula o alfa a partir do epsilon, logo o pico dura ~1 bloco.
#
# NOTA: isto era, ate agora, um efeito colateral NAO INTENCIONAL da ordem das
# instrucoes (o script repunha o alfa depois do load_model). Foi medido e o efeito e
# neutro-a-benefico: o bloco 1 das sessoes tem WR +0,43pp face aos restantes (0,6
# desvios, dentro do ruido), sem qualquer queda sistematica na fronteira das sessoes.
# Passa a ser DELIBERADO e controlavel. Poe None para desligar.
WARM_RESTART_ALPHA = ALPHA_START
BATTLE_FORMAT = "gen9nationaldex"
BLOCO_VALIDACAO = 2_000      # batalhas de avaliacao gulosa (epsilon=0) no fim
AGENT = "ash"
...
async def main():
    ...
    agent = AshAgent(
        account_configuration=AccountConfiguration("AshTrain", None),
    ...
    # Warm restart deliberado (ver WARM_RESTART_ALPHA). Se None, mantem o alfa
    # decaido que veio do .pkl.
    if WARM_RESTART_ALPHA is not None:
        agent.brain.alpha = WARM_RESTART_ALPHA
    ...
    rel = RelatorioConsola(agente="ASH", descricao="Q-puro (Q-Learning sem instinto)")
    rel.cabecalho(
        config={
    ...
    while total < MAX_BATALHAS:
        ...
        if not ok_div:
            print()
            print("!" * 70)
            print(f"  [%s] TREINO ABORTADO — DIVERGENCIA DETETADA" % "ASH")
            print(f"  {msg_div}")
    ...
    try:
        generate_graph(csv_path, graf_path,
                       agent="Ash", opponent="Instinto", total_battles=total,
    ...
    # ------------------------------------------------------------------
    # VALIDACAO GULOSA (epsilon = 0)
    # ------------------------------------------------------------------
    # Durante o treino o epsilon fica em MIN_EPSILON, logo ~5% das jogadas sao
    # deliberadamente aleatorias e o WR medido subestima a politica aprendida
    # (pela sensibilidade medida no projeto, ~82pp por unidade de epsilon, o custo
    # e ~4pp). Aqui medimos a politica PURA, que e o numero que interessa a tese.
    #
    # O epsilon de treino e restaurado no fim: o proximo ciclo continua a explorar.
    if BLOCO_VALIDACAO > 0:
        eps_treino = agent.brain.epsilon
        alpha_treino = agent.brain.alpha
        try:
            agent.brain.epsilon = 0.0
            agent.brain.alpha = 0.0        # nao aprender durante a avaliacao
            agent.reset_battles()
            await agent.battle_against(opponent, n_battles=BLOCO_VALIDACAO)
            wr_guloso = agent.n_won_battles / max(1, agent.n_finished_battles) * 100.0
            m_val = agent.pop_block_metrics()
            resultados["Validacao (eps=0)"] = (
                f"{wr_guloso:.2f}% em {BLOCO_VALIDACAO:,} batalhas "
                f"| margem {m_val.get('margem_media', 0):.2f} "
                f"| duracao {m_val.get('duracao_media', 0):.0f}t")
            resultados["Ganho vs treino"] = f"{wr_guloso - (recent_wr[-1] if recent_wr else 0):+.2f} pp"
        except Exception as e:
            resultados["Validacao (eps=0)"] = f"FALHOU: {e}"
        finally:
            # Restaura o estado de treino ANTES de salvar, senao o cerebro ficaria
            # gravado com epsilon=0 e deixaria de explorar para sempre.
            agent.brain.epsilon = eps_treino
            agent.brain.alpha = alpha_treino
            agent.save_brain()
